...
class repoManager():
    def __init__(self, repo_url):
        ...
        try:
            ...
            t1 = time.time()
            log.error('Failed to connect to helm chart repo '+self.repo_url + ' after '+ str(settings.HTTP_RETRY) + ' retries and ' + str(t1 - t0) + ' seconds. (Caused by: ' +  err.__class__.__name__+ ')')
        else:
    ...
    def get_index(self):
        try:
            ...
            if response.status_code != 200:
                raise RepoManagerError("Get helm repo index failed. "+ str(response.status_code)  +" "+ response.content.decode("utf-8"), response.status_code)
    ...
    def upload_chart(self, xapp):
        ...
        try:
            ...
            if response.status_code != 201:
                raise RepoManagerError("Upload helm chart failed. Status code: "+ str(response.status_code)  +" "+ response.content.decode("utf-8"), response.status_code)
        ...
        try:
            ...
            if response.status_code != 200:
                ...
                if xapp.chart_name+'-'+xapp.chart_version+'.tgz' not in response_dict["error"]:
                    raise RepoManagerError("Delete helm chart failed. " + str(response.status_code)  +" "+ response.content.decode("utf-8"),response.status_code)
    ...
    def get_xapp_list(self, xapp_chart_name=None):
        ...
        try:
            ...
            if response.status_code != 200:
                raise RepoManagerError("Get xApp charts list failed. "+ str(response.status_code)  +" "+ response.content.decode("utf-8"), response.status_code)
    ...
    def download_xapp_chart(self, xapp_chart_name, version):
        ...
        try:
            ...
            if response.status_code != 200:
                raise RepoManagerError("Download helm chart failed. "+ str(response.status_code)  +" "+ response.content.decode("utf-8"), response.status_code)
